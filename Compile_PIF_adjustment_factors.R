...
# adding the BBS "aou" numbers incase useful.
for(i in 1:nrow(Rosen_pif)){
  Rosen_pif[i,"aou"] <- as.integer(bbsBayes2::search_species(Rosen_pif$cn[i])[1,"aou"])
...
PIF_adjs <- ExpAdjs %>%
  select(cn,Aou, Species, TimeAdj.meanlog,Dist2,Pair2,
         edr,availability) %>%
  mutate(C_t = exp(TimeAdj.meanlog),
         C_t_avail = (1/availability),
         C_d = (1/(50*(Dist2/1000)^2*3.14159)), # area within detection distance of 50 BBS stops (square km)
