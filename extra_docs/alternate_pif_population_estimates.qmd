...
To account for the uncertainty of this calibration, we can fit a Bayesian hierarchical model that estimates the calibration factor while accounting for: 1) the variation in the calibration factor among routes, 2) the variation in the calibration factor among repeated counts on each route; 3) the estimated availability of birds for detection (and its uncertainty) during a 3-minute BBS point count conducted on that day and in that BBS survey stratum; 4) the estimated detectability distance for roadside counts and the uncertainty in its estimate; and 5) the pair correction factor used in previous PIF population estimates [@stanton2019].

The model would use the area weighted mean eBird relative abundance cell values in the buffer zone of route-j ($RelativeAbundance_j$, same as above) to model the observed BBS count on route-j, by observer-i, in year-t.
