...
To complement this BBS-adjacent population density, we can estimate a relative density of the eBird abundance within a buffer surrounding the BBS route paths, as the mean of the relative abundance per unit area across the same $M$ BBS routes.

$$
RelativeDensity_{BBSAdjacent} = \sum_{j=i}^{j=M} \frac{RelativeAbundance_{i j}}{BufferArea_{i j}}
$$
...
Without going into specifics, we can estimate the uncertainty of this scaling factor by assessing the variation among routes (bootstrapping), the variation in the estimated relative abundance surface, in addition to the uncertainties of the correction factors included in the original PIF approach.
