...
Both the BBS counts and the values of the eBird weekly abundance surfaces are designed to represent relative abundance: abundance at a given location, relative to other locations. Each source of information is designed to be an index of abundance: the counts during BBS ($C_{bbs}$) through the methodological controls in the survey protocol; or the abundance indices ($I_{ebird}$) through the analytical adjustments in the eBird analyses. These indices of abundance are by definition, proportional to true abundance ($N$), and so each index has some associated proportionality constant ($a_{bbs}$ or $a_{ebird}$). The PIF population estimation equation [@stanton2019] represents one estimate of the BBS proportionality constant $a_{bbs}$, i.e., a factor that scales the observed counts by BBS observers to true abundance.
...
$$
Combining these equations, it becomes clear that because each index is proportional to true abundance, there is also a proportional relationship between the two indices and their proportionality constants.
$$
N = \frac{C_{bbs}}{a_{bbs}}  = \frac{I_{ebird}}{a_{ebird}} 
$$
Further re-arranging shows a relationship for predicting the BBS counts based on the ebird abundance indices and the quotient of their proportionality constants.
...
$$
So finally, the right side of this equation gives the basis for estimating $B$ using the relationship between the ebird relative abundance surface and the observed BBS counts. 
...
The re-scaling to represent density accounts for: 4) the estimated availability of birds for detection (and its uncertainty) during a 3-minute BBS point count conducted on that day; 5) the estimated detectability distance for roadside counts and the uncertainty in its estimate; and 6) a correction factor used in previous PIF population estimates to account for the bias in the detectability between male and female birds of each species [@stanton2019]. 

We can then combine the posterior draws of the exponentiated scaling factor $B$, with Monte Carlo processes similar to those used in @stanton2019 to propagate the uncertainty of the scaling factor with the estimates of correction factors that match BBS observation conditions. To account for the asymmetries of the log-scale retransformation, we added the $0.5*(\sigma_{B}/a)$ component to approximate the mean of the exponentiated heavy-tailed log-t distribution [@link2017], where $a = \frac{(1.422*(\nu^{0.906}))}{(1+(1.422*(\nu^{0.906})))}$.

$$Calibration = \frac{exp(B + 0.5*(\sigma_{B}/a)) * C_p * C_t}{area}$$
