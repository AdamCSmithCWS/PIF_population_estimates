...
data {
  int<lower=1> n_route_obs; // number of BBS observer and route combinations in species seasonal range
  ...
  vector[n_route_obs] log_mean_rel_abund_sd; // sd of the log-transformed mean relative abund within BBS route buffers
...
parameters {
  ...
  vector[n_route_obs] beta_raw; // route-specific calibrations
  real<lower=1> nu;

  vector[n_counts*use_pois] noise_raw; // over-dispersion if use_pois == 1
  real<lower=0> sdnoise;    // sd of over-dispersion, if use_pois == 1

  vector[n_route_obs] true_log_mean_rel_abund; // log mean relative abundance with meausrement error
...
// replacing the strata level difference values
    ...
    E[i] = beta[route[i]] + doy_pred[doy[i],strata[i]] + yeareffect[strata[i],year[i]] + true_log_mean_rel_abund[route[i]] + noise;
  }
...
model {
  ...
  sum(beta_raw) ~ normal(0,0.001*n_route_obs); // soft sum to zero constraint

  for(j in 1:n_route_obs){
    log_mean_rel_abund[j] ~ normal(true_log_mean_rel_abund[j],log_mean_rel_abund_sd[j]);
  }

  sd_beta ~ student_t(3,0,1);
  //sd_beta ~ student_t(3,0,0.1);
